...
void maze::mapMazeToGraph(Graph &g) {
	...
	for (int rows = 0; rows < value.rows(); rows++) { // for all rows exccept far right
		for (int cols = 0; cols < value.cols(); cols++) { // for all columns in all rows
			if (value[rows][cols] == true) { // if there is a term at the designated spot
				if (cols +1 < value.cols()) { // if there is also a term one to the right of the found termm
					if (value[rows][cols + 1] == true) {
						...
						vItr2 = vItr1;
						vItr2++;
						boost::add_edge(*vItr1, *vItr2, g); // set edge connecting the next cell
						boost::add_edge(*vItr2, *vItr1, g); // set reverse edge
					}
				}
				if (rows +1 < value.rows()) { // if there is a term below the designated spot
					if (value[rows + 1][cols] == true){
