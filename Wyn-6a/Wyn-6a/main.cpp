...
void msfPrim(Graph &g, Graph &sf) {
	HeapV <Graph::vertex_iterator, Graph> pointHeap;
	pair<Graph::vertex_iterator, Graph::vertex_iterator> vItrRange = vertices(g); // find vertices range
	...
	while (pointHeap.size() != 0) { // while nodes remain in heap
		...
		for (Graph::adjacency_iterator eItr = vItrRange.first; eItr != vItrRange.second; eItr++) { // for all edges at this node
			targetNode = target(*eItr, g); // find the target node
			if ((g[targetNode].weight > g[*eItr].weight) && (g[targetNode].marked == false)) {	
				g[targetNode].weight = g[*eItr].weight;
...
int main()
{
	try
	{
		...
		if (connected) cout << "Forest is connected" << endl;
		else cout << "Forest is not connected" << endl;
		if (cyclic) cout << "Forest is cyclic, somethign went wrong" << endl << endl;
		else cout << "Forest is not cyclic" << endl << endl;
		...
		cout << "sum of all weights is " << weightSum << endl << endl; // print edge weights
		getchar();
	}
	catch (...) {
		getchar();
